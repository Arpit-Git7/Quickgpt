...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  const { chats, setSelectedChat, theme, setTheme, user, navigate,createNewChat,api,setChats,fetchUserChats,setToken,token } = useAppContext()
  ...
  const delterChat=async(e,chatId)=>{
    try{
      e.stopPropagation()
      const confirm=window.confirm("Are you sure you want to delete this chat?")
      if(!confirm) return;
      const {data}=await api.post('/api/chat/delete/',{chatId},{headers:{Authorization:token}})
      if(data.success){
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 
      ...
      />
      {/*new Chat Button*/}
      <button onClick={createNewChat} className='flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cusrsor-pointer'>
      ...
      {/* Search Convo */}
        <div className="flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md">
          ...
          <input
            ...
            className="flex-1 text-xs placeholder:text-gray-400 bg-transparent outline-none text-black dark:text-white"
          />
        </div>
      {/*Recent Chats*/}
        ...
        {chats
          ...
          .map((chat) => (
            ...
            >
              ...
              <img
                onClick={e=> toast.promise(delterChat(e,chat._id),{loading:'deleting...'})}
                src={assets.bin_icon}
