...
const Credits = () => {
  ...
  const fetchPlans=async()=>{
    try{
      const {data}=await api.get('/api/credit/plan',{headers:{Authorization:token}})  
      if(data.success){
        setPlans(data.plans)
      }else{
  ...
  try{
    const {data}=await api.post('/api/credit/purchase',{planId},{headers:{Authorization:token}})
    ...
    console.error('Purchase error:', error) // Add this line
    toast.error(error.message)
  }
}
